...
const REPORT_CONFIGS: Record<Tab, { endpoint: string; columns: any[] }> = {
  ...
  kpis: {
    ...
    columns: [
      ...
      { key: 'target_value',            label: 'Target',   right: true, format: (v: number) => Number(v).toLocaleString() },
      { key: 'current_value',           label: 'Achieved', right: true, format: (v: number) => Number(v).toLocaleString() },
      { key: 'unit',                    label: 'Unit' },
      { key: 'target_date',             label: 'Target Date', format: (v: string) => formatDate(v) },
      { key: 'project.name',            label: 'Project' },
      { key: 'project.programme.name',  label: 'Programme' },
    ],
  },
  audit: {
